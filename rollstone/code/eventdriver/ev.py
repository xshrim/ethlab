...
class User:
    def __init__(self, addr, uid, eid, uhash, level, timestamp, status, mid, txhash, tchash):
        self.addr = addr
        self.uid = uid
        self.eid = eid
        self.uhash = uhash
        ...
        self.status = status
        self.mid = mid
        self.txhash = txhash          # 生成时的交易hash (下同)
        self.tchash = tchash          # 最后一次发生变化时的交易hash (下同)
...
class Item:
    def __init__(self, iid, tid, uperid, userid, xhash, shash, ihash, cipher, ikey, iopen, level, timestamp, status, mid, txhash, tchash):
        self.iid = iid
        self.tid = tid
        self.uperid = uperid
        self.userid = userid
...
class Log:
    def __init__(self, sn, userid, itemid, perms, operate, senderid, sender, duration, timestamp, details, mid, txhash, tchash):
        self.sn = sn
        self.userid = userid
        self.itemid = itemid
        self.perms = perms
...
def getUser(userid, mid, txhash):
    try:
        ...
        user = User(addr, uid, eid, uhash, level, timestamp, status, mid, txhash, txhash)
...
def getItem(itemid, mid, txhash):
    try:
        ...
        item = Item(iid, tid, uperid, userid, xhash, shash, ihash, cipher, ikey, iopen, level, timestamp, status, mid, txhash, txhash)
...
def getPerm(permid, mid, txhash):
    try:
        ...
        perm = Perm(pid, tid, sgerid, userid, itemid, phash, device, pmark, prvs, ptime, ptimes, pslice, ptimestamp, ptype, status, mid, txhash, txhash)
...
def parseReceipt(receipt, mid):
    try:
        if receipt is not None and len(receipt) > 0:
            ...
            mid = mid/10000.0
            log = Log(sn, userid, itemid, perms, operate, senderid, sender, duration, timestamp, details, mid, txhash, txhash)
            jslogs = [log.show()]
            jsusers = []
            jsitems = []
            jsperms = []
            ...
            if operate.startswith('addUser') or operate.startswith('setUser') or operate.startswith('setMyHash'):
                ...
                jsuser = user.show()
                if not operate.startswith('addUser'):
                    del jsuser["mid"]
                    del jsuser["txhash"]
                jsusers.append(jsuser)
                ...
                jsitem = item.show()
                if not operate.startswith('addItem'):
                    del jsitem["mid"]
                    del jsitem["txhash"]
                jsitems.append(jsitem)
                # res = write({"item": [jsitem], "log": [jslog]})
            elif operate.startswith('addPerm') or operate.startswith('setPerm'):
                perm = getPerm(perms[0], mid, txhash)
                jsperm = perm.show()
                if not operate.startswith('addPerm'):
                    del jsperm["mid"]
                    del jsperm["txhash"]
                jsperms.append(jsperm)
                pperm = getPerm(perm.tid, mid, txhash)
                if pperm is None:
                    jspperm = pperm.show()
                    del jspperm["mid"]
                    del jspperm["txhash"]
                    jsperms.append(jspperm)

                # res = write({"perm": [perm.show(), pperm.show()], "log": [log.show()]})
            else:
                for i in range(len(perms)):
                    perm = getPerm(perms[i], mid, txhash)
                    if perm is not None:
                        jsperm = perm.show()
                        del jsperm["mid"]
                        del jsperm["txhash"]
            ...
            if not write({"user": jsusers, "item": jsitems, "perm": jsperms, "log": jslogs}):
...
def main():
    ...
    db = condb("172.17.0.2", 9927, "myrs", "rsdb")
    # db = condb("172.16.201.189", 9927, "myrs", "rsdb")
    # write()

    initNode("172.17.0.2", "root", "root", "ws://172.17.0.2:8516", "/data/ethdata/node1/keystore")
    ...
    contractPath = os.path.join(os.path.split(os.path.realpath(__file__))[0], 'RS.json')

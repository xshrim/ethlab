...
def createTransaction(itransaction):
    ...
    bl = itransaction['blockNumber'] if itransaction['blockNumber'] is not None else -1
    transaction = Transaction(
        str(w3.toHex(itransaction['blockHash'])), int(bl), str(itransaction['from']), str(to), int(itransaction['value']),
        int(itransaction['gas']), int(itransaction['gasPrice']), str(w3.toHex(itransaction['hash'])), str(itransaction['input']),
        int(itransaction['nonce']), int(itransaction['transactionIndex']), str(w3.toHex(itransaction['r'])),
        str(w3.toHex(itransaction['s'])), int(itransaction['v']))
    return transaction
...
def main():
    worker = threading.Thread(target=log_loop, args=(block_filter, 1))
    worker.start()
...
def background_thread():
    ...
    while True:
        ...
        socketio.emit('my response', {'data': str(t)}, namespace='/test')  # 注意：这里不需要客户端连接的上下文，默认 broadcast = True ！！！！！！！

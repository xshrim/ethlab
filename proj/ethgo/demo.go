...
// 使用ganache内存链
// gasPrice=0 交易无手续费
ganache-cli --accounts=5 --defaultBalanceEther=10000 --gasPrice=0 --gasLimit 0xfffffffffff --allowUnlimitedContractSize
...
func getAddressFromPrivateKey(hexkey string) (string, error) {
...
func getAuth(client *ethclient.Client, hexkey, value, gaslimit string) (*bind.TransactOpts, error) {
	privateKey, err := crypto.HexToECDSA(hexkey)
	if err != nil {
		return nil, err
	}
	...
	gasLimit, err := strconv.ParseUint(gaslimit, 10, 64) // in units
	if err != nil {
		return nil, err
	}
	...
	//auth.GasPrice = new(big.Int).Div(gasPrice, new(big.Int).SetInt64(int64(100000000)))
	auth.GasPrice = gasPrice

	return auth, nil
}

func deployContract(client *ethclient.Client, hexkey, cabi, cbin, value, gaslimit string, args ...interface{}) (common.Address, *types.Transaction, *bind.BoundContract, error) {

	auth, err := getAuth(client, hexkey, value, gaslimit)
	if err != nil {
	...
	if len(args) > 0 {
		return bind.DeployContract(auth, parsed, common.FromHex(cbin), client, args)
	} else {
		return bind.DeployContract(auth, parsed, common.FromHex(cbin), client)
	}

	//address, tx, contract, err := bind.DeployContract(auth, parsed, common.FromHex(cbin), eclient, version)
	//if err != nil {
	//	return common.Address{}, nil, nil, err
	//}
...
func main() {
	...
	// addr := "0x3134344fd035117f4b8b2a9ac71c2c1dd91f334f"
	hexkey := "2c0d45c96997c820f99d6957b0f57804ae2027444025aa893366b3c27ef99e83"
	hexkey = strings.TrimLeft(hexkey, "0x")

	addr, err := getAddressFromPrivateKey(hexkey)
	if err != nil {
		log.Fatal(err)
	}

	eclient, err := getClientEth("http://127.0.0.1:8545")
	balance, err := getBalanceEth(eclient, addr)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(balance)

	cc, err := compileContract("./Coin.sol") // compile solidity source file
	if err != nil {
		log.Fatal(err)
	}

	for k, v := range cc {
		fmt.Println(hexkey, " Deploying Contract ", k)

		cver := "1.0"
		cbin := v.Code

		abi, _ := json.Marshal(v.Info.AbiDefinition)
		cabi := string(abi)
		...
		//aa := []byte{}
		//fmt.Println(abii.UnmarshalJSON(aa))
		//fmt.Println(string(aa))

		caddr, tx, bc, err := deployContract(eclient, hexkey, cabi, cbin, "0", "10000000", cver)
		if err != nil {
			log.Fatal(err)
		}

		fmt.Println(caddr, tx, bc)

		ckey := [32]byte{}
		copy(ckey[:], []byte("redhat"))
		cvalue := "OK"
		res, err := callBC(bc, addr, "show", ckey, cvalue)
		if err != nil {
			log.Fatal(err)
		}

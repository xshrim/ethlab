...
def codegen(contract, output, abigen):
    ...
    if str(abigen.split(".")[-1]).lower() == "go":      # for golang
        if abigen.lower() == "go":
            gofile = os.path.join(output, str(contract['name']).lower() + '.go')
        else:
            gofile = os.path.join(output, abigen)
        cmd = "abigen --bin=%s --abi=%s --type %s --pkg=%s --out=%s" % (binfile, abifile, contract["name"], contract["name"].lower(), gofile)
    else:                                               # for java
        cmd = "web3j solidity generate %s %s -o %s -p %s" % (binfile, abifile, output, abigen)
    os.system(cmd)
    os.remove(abifile)
    os.remove(binfile)
...
def compiler(source, output, java, mode='file'):
    ...
    try:
        ...
        if abigen != '':
            codegen(contracts[-1], output, abigen)
        if output != '':
...
def usage(mode):
    if mode == 'compile':
        print("Usage:%s -c/--compile -f/--fname <contract filename> [-o/--output <output file>] [-a/--abigen <abigen package>]" % sys.argv[0])
    elif mode == 'deploy':
        print(
            "Usage:%s -d/--deploy -u/--url <ethereum rpc> -f/--fname <contract filename> [-o/--output <output file>] [-p/--package <compiled package>] [-v/--value <value>]" % sys.argv[0])
    elif mode == 'invoke':
        ...
        print(
            "Format:%s [-c/--compile] [-d/--deploy] [-i/--invoke] [-m/--monitor] [-s/--status] [-q/--query] [-p/--param] [-o/--output] [-a/--abigen]..."
            % sys.argv[0])
        print('')
        print("Usage:%s -c/--compile -f/--fname <contract filename> [-o/--output <output file>] [-a/--abigen <abigen package>]" % sys.argv[0])
        print(
            "Usage:%s -d/--deploy -u/--url <ethereum rpc> -f/--fname <contract filename> [-o/--output <output file>] [-a/--abigen <abigen package>] [-v/--value <value>]" % sys.argv[0])
        print("Usage:%s -i/--invoke -u/--url <ethereum rpc> [-t/--times <invoke times>]" % sys.argv[0])
        print("Usage:%s -m/--monitor -u/--url <ethereum rpc>" % sys.argv[0])
        print("Usage:%s -s/--status -u/--url <ethereum rpc>" % sys.argv[0])
        print("Usage:%s -q/--query -u/--url <ethereum rpc> -p/--param <query param>" % sys.argv[0])
...
if "__main__" == __name__:
    ...
    param = ''
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hcdimsqt:f:o:g:a:p:u:v:", [
            "help", "compile", "deploy", "invoke", "monitor", "status", "query", "times=", "file=", "output=", "gas=", "abigen=",
            "param=", "url=", "value="
        ])
        ...
        if action == 'compile':
            if fname != '':
                try:
                    ...
                    contracts = compiler(fname, output, abigen, 'file')
        ...
        if action == 'deploy':
            if fname != '' and url != '':
                try:
                    ...
                    contractInfo = deploy(w3, contracts, params, output, abigen)
            ...
            if url != '':
                try:
                    ...
                    print('collecting info ...'.ljust(25) + '<' +
                          time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) + '>')
                    show(info(w3))
                    print('collect done...'.ljust(25) + '<' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) +
                          '>')
                except Exception as ex:

...
contract Coop {
    ...
    */
    ...
    //@return 0: 是否执行成功
    */
    function publish(bytes32 pid, string memory title, string memory cata, string memory desc, uint pratio) public returns (bool) {
        ...
        projects[id] = Project(id, pid, sponsor, title, cata, desc, pratio, new bytes32[](0), new address[](0), 1, block.timestamp);
    ...
    */
    ...
    //@return 4: 项目类别
    //@return 5: 项目描述
    //@return 6: 项目售价(子项目分成比例)
    //@return 7: 项目子项目数
    //@return 8: 项目认领申请者数
    //@return 9: 项目状态
    //@return 10: 项目时间戳
    */
    function getProject(bytes32 id) public view returns (bytes32, bytes32, address, string memory, string memory, string memory, uint, uint, uint, uint, uint) {
        Project memory proj = projects[id];
        return (proj.id, proj.pid, proj.sponsor, proj.title, proj.cata, proj.desc, proj.pratio, proj.modules.length, proj.claimers.length, proj.status, proj.timestamp);
    }
